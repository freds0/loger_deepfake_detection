...
from src.data.manifest_utils import deterministic_split, write_manifest
...
def load_id_to_split(splits_dir: str, video_root: str) -> dict[str, str]:
    ...
    Falls back to a deterministic id-hash split (80/10/10) when the official
    ``{train,val,test}.json`` files aren't available locally.
    """
    if all(os.path.isfile(os.path.join(splits_dir, f"{s}.json")) for s in ("train", "val", "test")):
    ...
    print(f"[warn] no official split JSONs under {splits_dir}; falling back to a "
          f"deterministic id-hash split (80/10/10)")
    real_dir = os.path.join(video_root, "real")
    real_ids = [Path(p).stem for p in os.listdir(real_dir) if p.endswith(".mp4")]
    return {rid: deterministic_split(rid, salt="ffpp") for rid in real_ids}
...
def main() -> None:
    args = parse_args()
    id_to_split = load_id_to_split(args.splits_dir, args.video_root)
